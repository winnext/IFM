...
async findOneById(id: string) {
  let findOneNode= await this.neo4jService.findByIdWithTreeStructure(id);
  if (!findOneNode) {
...
let nodes = await this.neo4jService.findAllByClassName(data);
...
async update(_id: string, updateFacilityStructureDto: UpdateFacilityStructureDto) {
  ...
  let updatedNode = await this.neo4jService.updateById(_id,dynamicObject);
  if (!updatedNode) {
...
async findOneNodeByKey(key: string) {
  try {
    let node = await this.neo4jService.findOneNodeByKey(key);
    if (!node) {
