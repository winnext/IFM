...
import { RoomTopics } from '../const/kafta.topic.enum';
    ...
    switch (exception.getStatus()) {
      case 400:
        try {
          ...
          await this.postKafka.producerSendMessage(RoomTopics.ROOM_EXCEPTIONS, JSON.stringify(finalExcep));
          this.logger.warn(`${JSON.stringify(finalExcep)}   `);
          response.status(status).json(exception.getResponse());
        } catch (error) {
          console.log('`ROOM_EXCEPTION topic cannot connected due to ' + error);
        }
        break;
      case 401:
        try {
          ...
          await this.postKafka.producerSendMessage(RoomTopics.ROOM_EXCEPTIONS, JSON.stringify(finalExcep));
          console.log(`ROOM_EXCEPTION sending to topic from code 401`);
          this.logger.warn(`${JSON.stringify(finalExcep)}   `);
          response.status(status).json(clientResponse);
        } catch (error) {
          console.log('`ROOM_EXCEPTIONtopic cannot connected due to ' + error);
        }
        break;
      case 403:
        try {
